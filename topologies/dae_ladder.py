...
def build_model(input_shape, appliances):
    ...
    dense_2 = Dense(units=seq_length*MODEL_CONV_FILTERS/2, activation='relu')(dense_1)
	...
	#dense_4 = Dense(units=128, activation='relu')(dense_3)
    	dense_3 = Dense(units=seq_length*MODEL_CONV_FILTERS/2, activation='relu')(dense_2)
    	dense_4 = Dense(units=seq_length*MODEL_CONV_FILTERS, activation='relu')(dense_3)
    	#dense_4 = Reshape(target_shape=(seq_length, MODEL_CONV_FILTERS))(dense_4)
	#conv_3 = Conv1D(filters=16, kernel_size=MODEL_CONV_KERNEL_SIZE, strides=MODEL_CONV_STRIDES, padding=MODEL_CONV_PADDING, activation='linear')(conv_4)
	outputs_disaggregation.append(Conv1D(filters=1,
                     kernel_size=MODEL_CONV_KERNEL_SIZE,
                     strides=MODEL_CONV_STRIDES,
                     padding=MODEL_CONV_PADDING, activation='linear')(dense_4))
    # compile it!
